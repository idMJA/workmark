  ...
  const updateHistory = useCallback(async (newHistory: HistoryItem[]) => {
    try {
      await fetch('/api/history', {
        ...
        body: JSON.stringify([newHistory[0]]),
      });
      setHistory(prev => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const newHistory = [{ word: data.word, timestamp }, ...savedHistory].slice(0, 10);
        await fetch('/api/history', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      <div className="grid grid-rows-[20px_1fr_auto] items-center justify-items-center min-h-screen p-4 sm:p-8 pb-20 gap-8 sm:gap-16 sm:p-20">
              ...
              {definition && definition.meanings.length > 0 ? (
                <div className="flex flex-col gap-4 animate-slideUp w-full">
                  {definition.meanings.map((meaning, index) => (
                    <div key={`${word}-${meaning.partOfSpeech}-${index}`} className="text-left glass-effect p-6 rounded-xl hover:shadow-lg transition-all duration-300">
                      ...
                      <ul className="list-none">
                        {meaning.definitions.map((def, idx) => (
                          <li key={`${word}-${meaning.partOfSpeech}-${index}-${idx}`} className="text-lg mt-2 flex gap-3 hover:translate-x-1 transition-transform duration-200">
                            <i className="fas fa-angle-right text-green-400 mt-1.5" />
                            <span className="text-gray-100">{def.definition}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
        ...
        <footer className="row-start-3 flex flex-col items-center justify-center gap-1 sm:gap-2 text-gray-400 text-xs sm:text-sm glass-effect p-3 sm:p-4 rounded-xl w-full max-w-2xl animate-fadeIn">
          <p className="text-center px-2">
            Generated word can take some moment to appear, due to the API limitations.
          </p>
          <p className="flex flex-wrap items-center justify-center gap-x-2 gap-y-1 px-2 text-center">
            <span className="flex items-center gap-1">
              Made with <a href="https://nextjs.org" target="_blank" rel="noopener noreferrer" className="hover:text-green-500 transition-colors">Next.js</a>
            </span>
            <span className="flex items-center gap-1">
              by <a href="https://github.com/idMJA" target="_blank" rel="noopener noreferrer" className="hover:text-green-500 transition-colors">iaMJ</a>
            </span>
            <span className="hidden sm:inline">•</span>
            <a href="https://nwndev.github.io/wordmark" target="_blank" rel="noopener noreferrer" className="hover:text-green-500 transition-colors flex items-center gap-1">
              <i className="bi bi-arrow-90deg-right" />Originals
            </a>
            <span className="hidden sm:inline">•</span>
            <a href="https://github.com/idMJA/workmark" target="_blank" rel="noopener noreferrer" className="hover:text-green-500 transition-colors flex items-center gap-1">
              <i className="fas fa-code-branch" />Sources
            </a>
          </p>
        </footer>
      </div>
            ...
            <div className="p-6">
              ...
              {history.length > 0 ? (
                <ul className="divide-y divide-gray-700">
                  {history.map((item) => (
                    <li key={item.timestamp} className="py-3 flex justify-between items-center hover:bg-white/5 px-2 rounded transition-colors duration-200">
                      <span className="font-medium text-gradient">{item.word}</span>
                ...
                <p className="text-center text-gray-400 animate-pulse-slow">No history yet</p>
              )}
            </div>
          </div>
        </div>
